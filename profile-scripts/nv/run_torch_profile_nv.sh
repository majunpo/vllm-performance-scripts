...
LOG_DIR=${LOG_DIR:-./torch-profile-log}
TRACE_ROOT=${TRACE_ROOT:-./torch-profile-trace}
...
Usage: $0 [options]
  ...
  -b, --bs <n>                   decode batch size         (default: $BS)
      ...
      --enforce-eager            关闭 CUDA graph            (default: CUDA graph 开启)
      ...
      --trace-root <path>        torch trace output root   (default: $TRACE_ROOT)
      --check                    只做 CUDA 环境自检，不启动 profiling
  -h, --help
EOF
}
...
print('cuda available :', torch.cuda.is_available(), '| device_count:', torch.cuda.device_count())" 2>&1 | grep -v "^W[0-9]"
  exit 0
fi
...
LOG_FILE="$LOG_DIR/torch-profile-log-$TAG.log"
...
"$PYTHON_BIN" run_auto_model_nv-bs-decode-only.py \
  --model "$MODEL" \
  --tp "$TP" \
...
echo "[INFO] torch traces: $(realpath "$OUT_DIR")"